<script lang="ts">
	...
	import { menuNavLinks } from '$lib/navigation_link';
	...
	function setNavCategory(c: string): void {
		...
		switch($storeCategory) {
			case('compare'): filteredMenuNavLinks = menuNavLinks.filter((linkSet: any) => linkSet.id === 'compare'); break;
			case('check'): filteredMenuNavLinks = menuNavLinks.filter((linkSet: any) => linkSet.id === 'check'); break;
			case('settings'): filteredMenuNavLinks = menuNavLinks.filter((linkSet: any) => linkSet.id === 'settings'); break;
		}
	}
	// Lifecycle
	page.subscribe((p) => {
